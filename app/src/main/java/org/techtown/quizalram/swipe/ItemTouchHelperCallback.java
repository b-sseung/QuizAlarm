package org.techtown.quizalram.swipe;
...
public class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
            ...
            if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){ //스와이프 됫는지
                if(buttonsShowedState != ButtonsState.GONE){ //스와이프가 중간에 있는지 확인
                    ...
                    if(buttonsShowedState == ButtonsState.RIGHT_VISIBLE) {
                        dX = Math.min(dX, -buttonWidth); //중간이 아니라 오른쪽으로 스와이프 되어있으면
                        Log.d("2222", "클릭함 3");
                    }
    ...
    @Override public int convertToAbsoluteDirection(int flags, int layoutDirection) { //스와이프되서 화면에서 사라지는 것을 막아줌.
        if(swipeBack){
            swipeBack = false;
            return 0;
        }
    ...
    public static void setItemsClickable(RecyclerView recyclerView, boolean isClickable){
